...
export default function BlogPost({ postId }: PostDetails) {
  ...
  const customMarkdownComponents = {
    ...
    li: ({ node, ...props }: CustomComponentProps) => {
      ...
      if (React.isValidElement(firstChild) && typeof firstChild.props.children === 'string') {
        ...
        if (text.startsWith('[ ]') || text.startsWith('[x]') || text.startsWith('[X]')) {
          ...
          childrenArray[0] = React.cloneElement(firstChild, { children: (firstChild.props.children as string).slice(3).trim(),
          });
        }
      }
    ...
    blockquote: ({ node, ...props }: CustomComponentProps) => (
      <blockquote
        style={{ borderLeft: '5px solid #ccc', paddingLeft: '1rem', marginTop: '1rem' }}
        {...props}
      />
    ),
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (blogPostData === null) {
        ...
        const post = HomeData.blogapi.records.find((item) => item.id === postId);
        if (post) {
          setBlogPostData(post);
        }
        setLoading(false);
      ...
      {loading ? (
        <>
          <Box sx={{ background: '#F4F4F4', justifyContent: 'left', alignItems: 'left', alignContent: 'left', width: '100%', height: '100%', px: isSmallScreen ? '1.25rem' : isMediumScreen ? '4.5rem' : isLargeScreen ? '4.5rem' : isExtraLargeScreen ? '5.5rem' : '4.5rem', pb: isSmallScreen ? '2.5rem' : isMediumScreen ? '3rem' : isLargeScreen ? '4rem' : isExtraLargeScreen ? '4rem' : '4rem' }}>
            ...
            <Skeleton variant="text" sx={{ fontSize: '5rem', width: '60%' }} />
          ...
          <Box sx={{ background: '#fff', justifyContent: 'center', alignItems: 'center', alignContent: 'center', width: '100%', height: '100%', px: isSmallScreen ? '1.25rem' : isMediumScreen ? '4.5rem' : isLargeScreen ? '4.5rem' : isExtraLargeScreen ? '5.5rem' : '4.5rem', pt: isSmallScreen ? '2rem' : isMediumScreen ? '3rem' : isLargeScreen ? '3rem' : isExtraLargeScreen ? '3rem' : '3rem', pb: isSmallScreen ? '1rem' : '2rem' }}>
            <Skeleton variant="rounded" sx={{ marginLeft: '1rem', marginRight: '1rem', width: '100%', height: '400px', objectFit: 'contain', borderRadius: '12px' }} />
          </Box>
          <Box sx={{ background: '#fff', justifyContent: 'left', alignItems: 'left', alignContent: 'left', width: '100%', height: '100%', px: isSmallScreen ? '1.25rem' : isMediumScreen ? '4.5rem' : isLargeScreen ? '4.5rem' : isExtraLargeScreen ? '5.5rem' : '4.5rem', pt: isSmallScreen ? '1rem' : '2rem', pb: isSmallScreen ? '2.5rem' : isMediumScreen ? '3rem' : isLargeScreen ? '4rem' : isExtraLargeScreen ? '4rem' : '4rem' }}>
            <Skeleton variant="text" sx={{ fontSize: '5rem', width: '40%' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '100%' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '80%' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '100%', mt: '2rem' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '80%' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '100%', mt: '2rem' }} />
            <Skeleton variant="text" sx={{ fontSize: '2rem', width: '80%' }} />
          </Box>
        ...
        <>
          ...
          <Box sx={{ background: '#fff', justifyContent: 'center', alignItems: 'center', alignContent: 'center', width: '100%', height: '100%', px: isSmallScreen ? '1.25rem' : isMediumScreen ? '4.5rem' : isLargeScreen ? '4.5rem' : isExtraLargeScreen ? '5.5rem' : '4.5rem', pt: isSmallScreen ? '1rem' : '2rem', pb: isSmallScreen ? '2.5rem' : isMediumScreen ? '3rem' : isLargeScreen ? '4rem' : isExtraLargeScreen ? '4rem' : '4rem' }}>
               {/* @ts-ignore */}
            <Markdown children={blogPostData.fields.Content} remarkPlugins={[remarkGfm]} components={customMarkdownComponents} />
          </Box>
        </>
      )}
    </>
