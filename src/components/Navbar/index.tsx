...
export default function StoreNavBar({ activeComponent }: NavBarProps) {
  ...
  return (
    <>
      <link href="https://cdn.jsdelivr.net/npm/@vetixy/circular-std@1.0.0/dist/index.min.css"
            rel="stylesheet"/>
      ...
      <AppBar
        ...
        sx={{
          ...
          pt: '0.5rem',
          pb: '0.75rem',
          px: isMobile ? '1rem' : (isMediumScreen ? '3rem' : (isLargeScreen ? '3rem' : (isExtraLargeScreen ? '3rem' : '3rem'))),
      ...
      >
        <Toolbar sx={{p: '0.5rem'}}>
          ...
          >
            ...
            <List sx={{px: '1rem'}}>
              <Typography variant="h6" component="div"
                          sx={{flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: "center", mb: '2rem'}}>
                <Image src={"/assets/impneglogo.svg"} width={295} height={30} alt="logo"/>
              </Typography>
              {menuItems.map((item) => (
          ...
          {!isMobile && (
            <Box sx={{display: "flex", gap: isMediumScreen ? '1rem' : (isLargeScreen ? '2rem' : (isExtraLargeScreen ? '3rem' : '2rem'))}}>
              {menuItems.map((item) => (
