  ...
  return (
    <>
      <Box sx={{
        ...
        width: '100%',
        height: 'auto',
        px: isSmallScreen ? '1.25rem' : (isMediumScreen ? '4.5rem' : (isLargeScreen ? '4.5rem' : (isExtraLargeScreen ? '5.5rem' : '4.5rem')))
      }}>
        <Grid container spacing={{ xs: 2, md: 2, xl: 4 }} sx={{ flexWrap: 'wrap' }}>
          ...
          <Grid item key={'Herotext'} xs={12} md={8} lg={8} sx={{ mt: isSmallScreen ? '1.25rem' : '6%', display: 'flex', flexDirection: 'column', alignItems: isSmallScreen ? 'center' : 'start' }}>
            ...
            <Box sx={{ display: "flex", flexDirection: 'row', gap: '2rem' }}>
              ...
              <ButtonBase onClick={() => setModalOpen(true)} sx={{ // Open modal on click
                mt: '2rem',
                borderRadius: '12px',
                width: 'auto'
              }}>
      ...
      <FreeSampleModal open={modalOpen} handleClose={() => setModalOpen(false)} /> {/* Include modal */}
    </>
  );
};
