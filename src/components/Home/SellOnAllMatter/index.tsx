...
const Hero = () => {
  ...
  return (<>
    ...
    <Box sx={{ pb:'2rem',width:'100%', height: 'auto',px: isSmallScreen ? '1.25rem' : (isMediumScreen ? '4.5rem' : (isLargeScreen ? '4.5rem' : (isExtraLargeScreen ? '5.5rem' : '4.5rem')))}}>
  <Grid container spacing={{ xs:2, md:2, xl: 4 }} sx={{flexWrap: 'wrap-reverse',pt:isSmallScreen?'2rem':'6rem',justifyContent:'space-between',mb:'2rem'}}>
      ...
      Find a tool to address your goals
          ...
          <Typography variant="subtitle2" sx={{whiteSpace: isMediumScreen?'balance':'break-spaces',pr: isSmallScreen ? '0rem' : (isMediumScreen ? '10%' : (isLargeScreen ? '10%' : (isExtraLargeScreen ? '10%' : '10%'))),textAlign:isSmallScreen?'center':'left',fontFamily: 'classicsans', fontWeight: 'light', color: '#8A8A89', fontSize: isSmallScreen ? '14px' : '20px', mt:"0.5rem", }}>
          This toolkit translates key concepts and techniques into a set of tools to help address typical negotiation challenges. Start with the Negotiation Canvas and learn more about the other 50+ tools      
          </Typography>
