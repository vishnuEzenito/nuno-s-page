...
const Hero = () => {
  ...
  return (<>
    ...
    <Box sx={{  pt:'3rem',pb:'2rem',width:'100%', height: 'auto',px: isSmallScreen ? '1.25rem' : (isMediumScreen ? '4.5rem' : (isLargeScreen ? '4.5rem' : (isExtraLargeScreen ? '5.5rem' : '4.5rem')))}}>
        <Grid container spacing={{ xs:2, md:2, xl: 4 }} sx={{flexWrap: 'wrap'}}>
      
      <Grid item key={'Herotext'} xs={12} md={6} lg={6} sx={{mt:isSmallScreen ? '1.25rem' : '6%',display:'flex',flexDirection:'column',alignItems:isSmallScreen?'center':'start'}}>
      ...
      Unlock your negotiation potential and maximize impact.
      </Typography>
          <Typography variant="subtitle2" sx={{whiteSpace: isMediumScreen?'balance':'break-spaces',pr: isSmallScreen ? '0rem' : (isMediumScreen ? '10%' : (isLargeScreen ? '20%' : (isExtraLargeScreen ? '20%' : '10%'))),textAlign:isSmallScreen?'center':'left',fontFamily: 'classicsans', fontWeight: 'light', color: '#1c1c1c', fontSize: isSmallScreen ? '14px' : '20px', mt:"0.5rem", }}>
          Whether in business deals, policy negotiations, conflicts, or personal relationships, this practical guide equips you with empowering mindsets and tools to succeed.         
          </Typography>
  ...
  <Grid container spacing={{ xs:2, md:2, xl: 4 }} sx={{flexWrap: 'wrap-reverse',pt:isSmallScreen?'2rem':'6rem',justifyContent:'space-between'}}>
  ...
  /> 
      ...
      <Grid item key={'text2'} xs={12} md={8} lg={8} sx={{display:'flex',flexDirection:'column',justifyContent: 'center',alignItems:isSmallScreen?'center':'start'}}>
      <Typography variant="h1" sx={{textAlign:isSmallScreen?'center':'left',fontFamily: 'classicsans', fontWeight: "bold", color: '#333333', fontSize: isSmallScreen ? '28px' : (isMediumScreen ? '40px' : (isLargeScreen ? '45px' : (isExtraLargeScreen ? '45px' : '40rem'))) }}>
      What are Impact Negotiations? 
          ...
          Negotiation is any effort to persuade. We negotiate all the time, about everything, with everyone, since our birth. It is a learnable skill: systematic negotiation processes, tools, and skills can help us get what we truly value.
